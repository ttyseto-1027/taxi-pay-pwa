...
if (!config.enabled || !config.apiKey || config.apiKey === 'REPLACE_ME') {
  ...
  async function getAllowlist(user) {
    ...
    if (!snapshot.exists() || snapshot.data().enabled !== true) {
      throw new Error(
        'このGoogleアカウントはv1.2βの利用許可リストに登録されていません。'
      );
    }

    return { ref, data: snapshot.data(), email };
  }
  ...
  async function loadProfile(user) {
    ...
    if (String(profile.email || '').toLowerCase() !== emailOf(user)) {
      throw new Error('登録済みメールアドレスとGoogleアカウントが一致しません。');
    }
    ...
    if (
      ...
      allow.data.registeredUid !== user.uid
    ) {
      throw new Error('利用許可情報が一致しません。管理者へお問い合わせください。');
    }
    ...
    return {
      ...profile,
      isAdmin: adminSnapshot.exists() && adminSnapshot.data().enabled !== false
    };
  }
  ...
  async function routeUser(user) {
    ...
    if (allow.data.invitationUsed === true || allow.data.registeredUid) {
      throw new Error(
        'このメールアドレスは登録済みですが、利用者情報を確認できません。管理者へお問い合わせください。'
      );
    }

    showInvite(user);
    setMessage(
      '許可済みアカウントです。初回のみ招待コードを入力してください。',
      'info'
    );
  }
  ...
  googleLoginButton.addEventListener('click', async () => {
    setBusy(true);
    setMessage('Googleアカウントを確認しています…', 'info');
    ...
    try {
      ...
      if (code === 'auth/popup-closed-by-user') {
        ...
        setMessage(
          'この公開URLがFirebaseの承認済みドメインに登録されていません。'
        );
      } else {
        setMessage(
  ...
  inviteCancelButton.addEventListener('click', async () => {
    await signOut(auth);
    showGate();
    setMessage('');
  });
  ...
  inviteForm.addEventListener('submit', async (event) => {
    ...
    if (!user) {
      setMessage('ログイン状態を確認できません。もう一度Googleログインしてください。');
      return;
    }
    ...
    if (!document.getElementById('inviteTerms').checked) {
      setMessage('利用条件とプライバシーポリシーへの同意が必要です。');
      return;
    }

    registrationInProgress = true;
    setBusy(true);
    setMessage('初回利用登録を確認しています…', 'info');
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        const allowSnapshot = await transaction.get(allowRef);
        const codeSnapshot = await transaction.get(codeRef);
        const userSnapshot = await transaction.get(userRef);

        if (!allowSnapshot.exists() || allowSnapshot.data().enabled !== true) {
          throw new Error(
            'このGoogleアカウントは利用許可リストに登録されていません。'
          );
        }

        const allow = allowSnapshot.data();

        if (allow.invitationUsed === true || allow.registeredUid) {
          throw new Error('このGoogleアカウントは既に初回登録済みです。');
        }

        if (userSnapshot.exists()) {
          throw new Error('このGoogleアカウントの利用者情報は既に存在します。');
        }
        ...
        if (code.version && code.version !== 'v1.2-beta') {
          throw new Error('この招待コードはv1.2β用ではありません。');
        }
        ...
        if (
          ...
          throw new Error('招待コードの設定が不正です。');
        }

        if (usageCount >= maxUses) {
          throw new Error('β版テスターの登録上限に達しています。');
        }
        ...
        transaction.set(userRef, {
          name: user.displayName || allow.displayName || '',
      ...
      if (!profile) {
        throw new Error('登録後の利用者情報を確認できませんでした。');
      }

      showApp(profile);
    } catch (error) {
      setMessage(error?.message || '初回利用登録に失敗しました。');
    } finally {
  ...
  getRedirectResult(auth).catch((error) => {
    setMessage(
      `Googleログインに失敗しました。${error?.code ? `（${error.code}）` : ''}`
    );
  });
