(() => {
  ...
  const I=window.TaxiPayInlineDiagnostic; I?.add('V6-BOOT-001','boot.js を開始しました。');
  ...
  I?.add('V6-MODULE-START','firebase-auth.js の読み込みを開始します。');
  import('./firebase-auth.js?v=20260725-6').then(mod=>{ I?.add('V6-MODULE-OK','firebase-auth.js を読み込みました。');
  ...
  }).then(()=>{I?.add('V6-AUTH-READY','認証機能の準備が完了しました。'); return D.record('APP-READY','info','認証機能の準備完了');}).catch(err=>{ I?.add('V6-MODULE-FAIL','認証機能の読み込みに失敗しました。',err);
