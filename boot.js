(() => {
  ...
  const I=window.TaxiPayInlineDiagnostic; I?.add('V14-BOOT-001','boot.js を開始しました。');
  ...
  I?.add('V14-MODULE-START','firebase-auth.js の読み込みを開始します。');
  import('./firebase-auth.js?v=20260727-14').then(mod=>{ I?.add('V14-MODULE-OK','firebase-auth.js を読み込みました。');
  ...
  }).then(()=>{I?.add('V14-AUTH-READY','認証機能の準備が完了しました。'); return D.record('APP-READY','info','認証機能の準備完了');}).catch(err=>{ I?.add('V14-MODULE-FAIL','認証機能の読み込みに失敗しました。',err);
